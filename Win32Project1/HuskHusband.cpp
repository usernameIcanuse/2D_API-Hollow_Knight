...
void CHuskHusband::Initialize()
{
	eID = OBJ_MONSTER;
	m_tInfo.fCX = 60.f;
	m_tInfo.fCY = 100.f;

	m_tImageInfo.fCX = 170.f;
	m_tImageInfo.fCY = 170.f;
	...
	m_eCurState = ST_IDLE;
	m_ePreState = ST_END;
	m_pFrameKey = L"HuskHusband_Left";
...
void CHuskHusband::Render(HDC hdc)
{
...
void CHuskHusband::Release()
{
}

int CHuskHusband::Update()
{
	if (m_bDead)
		return OBJ_DEAD;
...
void	CHuskHusband::Motion_Change()
{
...
void CHuskHusband::Collision()
{
}

void CHuskHusband::Collision(CObj * Obj)
{
		if (OBJ_WALL == Obj->Get_OBJID())
		{
			if (m_tInfo.fX < Obj->Get_Rect().left)//왼쪽 충돌
			{
				...
				m_tInfo.fX -= dx;
				m_fSpeed.fX *= -1.f;
				if (0 == lstrcmp(m_pFrameKey, L"HuskHusband_Left"))
					m_pFrameKey = L"HuskHusband_Right";
				else
					m_pFrameKey = L"HuskHusband_Left";
	...
	if (!m_bFreeze && !m_bDeath)
	{
		...
		if (OBJ_PLAYER == Obj->Get_OBJID())
		{
			...
			else
			{
				...
				else if (0 == lstrcmp(m_pFrameKey, L"HuskHusband_Right"))
					m_fSpeed.fX = 0.5f;
			}
		}
	}
